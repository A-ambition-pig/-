...
import com.nanfeng.gmall.realtime.bean.{BaseCategory3, BaseTrademark}
...
object BaseCategory3App {
  def main(args: Array[String]): Unit = {
    ...
    var topic = "ods_base_category3"
    var groupId = "dim_base_category3_group"
    ...
    offsetDStream.foreachRDD{
      rdd => {
        // 结构转换
        val baseTrademarkRDD: RDD[BaseCategory3] = rdd.map {
          record => {
            // 获取省份Json格式字符串
            val jsonStr: String = record.value()
            val baseCategory3: BaseCategory3 = JSON.parseObject(jsonStr, classOf[BaseCategory3])
            baseCategory3
          }
        }
